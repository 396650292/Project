...
<html>
  ...
  <%
		...
		//定义四个变量
		//数据总条数
		int dataCount=page1.queryCountByClub(club_name);
		//每页数据条数
		int count=5;
		//总页数
		int pages;
		...
		//数组传过去
		ArrayList<Member> list=page1.queryByPageByClub(cp*count,count,club_name);		
	%> 
  <body>
           ...
           <%
			...
			%>
					...
					<%if(cp==0){ %>
			  		...
			  		<%}else{ %>
			  		<a href="Members_query_byClub.jsp?currentPage=0" style="text-decoration: none">首页</a>
			  		<%}if(cp>0){ %>
					<a href="Members_query_byClub.jsp?currentPage=<%=cp-1 %>" style="text-decoration: none">上一页</a>
					<%}else{ %>
					<a style="color:gray; text-decoration: none"  >上一页</a>
					<%}if(cp<pages-1){
					 %>
					<a href="Members_query_byClub.jsp?currentPage=<%=cp+1 %>" style="text-decoration: none">下一页</a>
					<%}else{ %>
					<a style="color:gray; text-decoration: none"  >下一页</a>
					<%}if(cp<(pages-1)){ %>
					<a href="Members_query_byClub.jsp?currentPage=<%=pages-1 %>" style="text-decoration: none ">尾页</a>
					<%}else{ %>
